...
<c:if test="${ not empty ubicacionAplicacion.ruta }">
	...
	<li><fmt:message key="products.bc.etiqueta.crear" /></li>
</ul>
</div>
...
<div id="mainContent">
    ...
    <div id="iTitle" class="ui-state-default ui-widget-content ui-corner-all">
    	<fmt:message key="products.formulario.etiqueta.crear.titulo"/>
    </div>
...
<div id="action-bar">
    <button tabindex="7" id="btnGuardar" type="button" name="ok" onClick="doSubmit<c:if test="${ not empty detalle.cabecera || not empty esAsync}">Async</c:if>('products/crearEjecutar');"><fmt:message key="globales.formulario.boton.crear.guardar"/></button>
	<button tabindex="8" id="btnCancelar" type="button" name="cancel" onClick="doSubmit<c:if test="${ not empty detalle.cabecera || not empty esAsync}">Async</c:if>('products/cancelarCrear');"><fmt:message key="globales.formulario.boton.crear.cancelar"/></button>
</div>
</div>
<br />
