...
<c:if test="${ not empty ubicacionAplicacion.ruta }">
	...
	<li><fmt:message key="products.bc.etiqueta.ver" /></li>
</ul>
</div>
...
<div id="mainContent">
    ...
    <div id="iTitle" class="ui-state-default ui-widget-content ui-corner-all">
    	<fmt:message key="products.formulario.etiqueta.ver.titulo"/>
    </div>
...
<div id="action-bar">
	<button id="btnCancelar" type="button" name="regresar" onClick="doSubmit<c:if test="${ not empty detalle.cabecera}">Async</c:if>('products/cancelarVer');"><fmt:message key="globales.formulario.boton.regresar"/></button>
</div>
</div>
<br />
