...
<c:if test="${ not empty ubicacionAplicacion.ruta }">
	...
	<li><fmt:message key="categories.bc.etiqueta.crear" /></li>
</ul>
</div>
...
<div id="mainContent">
    ...
    <div id="iTitle" class="ui-state-default ui-widget-content ui-corner-all">
    	<fmt:message key="categories.formulario.etiqueta.crear.titulo"/>
    </div>
...
<div id="action-bar">
    <button tabindex="2" id="btnGuardar" type="button" name="ok" onClick="doSubmit<c:if test="${ not empty detalle.cabecera || not empty esAsync}">Async</c:if>('categories/crearEjecutar');"><fmt:message key="globales.formulario.boton.crear.guardar"/></button>
	<button tabindex="3" id="btnCancelar" type="button" name="cancel" onClick="doSubmit<c:if test="${ not empty detalle.cabecera || not empty esAsync}">Async</c:if>('categories/cancelarCrear');"><fmt:message key="globales.formulario.boton.crear.cancelar"/></button>
</div>
</div>
<br />
